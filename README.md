...
### Surge 经验
```
%APPDATA%\surge\settings.json # 路径

"default_download_dir"                     #  默认下载保存路径
"warn_on_duplicate": false,                #  下载重复文件时是否弹出警告
"download_complete_notification": false,   #  下载完成后是否显示系统通知
"clipboard_monitor": false,                #  是否监听剪贴板中的下载链接
"theme": 0,                                #  界面主题（0 = 默认/自动）
```
