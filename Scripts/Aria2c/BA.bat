...
if errorlevel 1 (
    echo aria2c 未找到，尝试在当前目录运行...
    if not exist "aria2c.exe" (
        echo 错误：找不到 aria2c.exe
        echo 请将 aria2c.exe 放在本脚本同目录下，或添加到系统PATH中。
        goto cleanup
    )
    set "aria2_cmd=aria2c.exe"
) else (
    set "aria2_cmd=aria2c"
)

rem 调用 aria2c 进行下载
!aria2_cmd! -i "!file_list!" --console-log-level=notice --continue=true --split=5 --max-connection-per-server=5

if errorlevel 1 (
    echo.
    echo aria2c 执行失败，返回错误码：!errorlevel!
) else (
    echo.
    echo 下载完成。
)
